class HandlersHelper {
  ...
  findIntentHandler(intent, path = []) {
     ...
     */
    let i = 0;
    while (typeof handler !== 'function' && i < 10) {
      ...
      if (handler === null) {
        ...
        if (baseHandlers['_ExitContext']) {
          if (baseHandlers['Unknown']) {
            blockPropogation = baseHandlers['_ExitContext'](this.session.data, this.session.user) === false;
          } else {
        ...
        if (!blockPropogation) {
          //look down a level
          level--;
          //console.log('Going down a level to ', path.slice(0, level));
     ...
     * - go down through each level, joing middleware so that the top level middleware is run first
     */
