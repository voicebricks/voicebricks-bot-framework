...
class Session {
    ...
    toIntent(intent, params, global) {
        ...
        try {
            return new Promise((resolve, reject) => {
                resolve(handler.handler.call(this, params));
    ...
    getRequestHandlers() {
        ...
        if (this.data.confirmIntent) {
            const confirmIntent = this.data.confirmIntent;
            ...
            Object.assign(level, {
                Yes: function () {
                    let intent = confirmIntent.intent;
                    delete this.data.confirmIntent;
                    this.toIntent(intent, confirmIntent.params);
                },
                No: function () {
                    const No = confirmIntent.No || {};
                    delete this.data.confirmIntent;
                    No.confirm && this.confirm(No.confirm);

                    if (No.toIntent) {
                        this.toIntent(No.toIntent);
                    } else {
                        this.ask();
                    }
                }
    ...
    message(key) {
        return new Promise((resolve, reject) => {
            this.bot.schema.Message.findOne({ key }, 'message', (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
                debug('Message for '+key, result);
